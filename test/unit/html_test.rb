...
describe RichText::HTML do
  ...
  it 'renders multiple level of inline attribution' do
    d = RichText::Delta.new([
                              { insert: 'a man ' },
                              { insert: 'a plan',
                                attributes: { bold: true, italic: true, link: 'https://visitpanama.com' } },
                              { insert: "\n" }
                            ])
    assert_equal '<p>a man <a href="https://visitpanama.com"><em><strong>a plan</strong></em></a></p>',
                 render_compact_html(d)
  end
  ...
  it 'renders custom object insertions using apply function' do
    ...
    f = {
      image: {
        tag: 'img',
        apply: ->(el, op, _ctx) { el[:src] = op.value.dig(:image, :src) }
      }
    }
  ...
  it 'renders object insertions with special block formats, referencing a known format' do
    ...
    assert_equal '<h1 id="bingo"><img src="https://placekitten.com/200/150"/></h1>',
                 render_compact_html(d, inline_formats: f)
  end
  ...
  it 'renders custom object insertions with special block formats, using a custom tag' do
    ...
    assert_equal '<figure id="bingo"><img src="https://placekitten.com/200/150"/></figure>',
                 render_compact_html(d, inline_formats: f)
  end
  ...
  it 'renders custom object insertions with a tag build function' do
    ...
    assert_equal '<figure id="bingo"><img src="https://placekitten.com/200/150"/><figcaption>cuteness</figcaption></figure>',
                 render_compact_html(d, inline_formats: f)
  end
  ...
  it 'renders custom object insertions into story flow' do
    ...
    assert_equal '<p>before</p><figure id="bingo"><img src="https://placekitten.com/200/150"/><figcaption>cuteness</figcaption></figure><p>after</p>',
                 render_compact_html(d, inline_formats: f)
  end
  ...
  it 'passes a render context argument to build and apply functions' do
    ...
    f = {
      image: {
        tag: lambda { |el, _op, ctx|
               build_context = ctx
               el
             },
        apply: ->(_el, _op, ctx) { apply_context = ctx }
      }
    }
  ...
  it 'applies tags then attributes, each ordered by priority' do
    ...
    assert_equal '<p><second id="second"><first>hitme</first></second></p>', render_compact_html(d, inline_formats: {
                                                                                                   ...
                                                                                                   attr1: { apply: lambda { |el, _op, _ctx|
                                                                                                                     el[:id] = 'first'
                                                                                                                   }, priority: 1 },
                                                                                                   attr2: { apply: lambda { |el, _op, _ctx|
                                                                                                                     el[:id] = 'second'
                                                                                                                   }, priority: 2 }
                                                                                                 })
  end
